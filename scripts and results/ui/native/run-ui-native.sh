...
run_top() {
    echo "run number $1"
    $adb shell "top -bd1 -o res,%cpu,%mem,args -m 200 | grep losalv.android | grep -v grep" >> ./measures$1.txt
}
...
for i in `seq 1 $TASK_REPETITIONS`
    ...
    $adb shell am start -W -n \
        "losalv.android.masterthesisnativeartifact/losalv.android.masterthesisnativeartifact.MainActivity" \
